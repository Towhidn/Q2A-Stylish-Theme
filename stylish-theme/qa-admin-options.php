...
class qa_html_theme_layer extends qa_html_theme_base {
	...
	function nav_list($navigation, $class, $level=null)
	{
		...
		if(count($navigation) > 1 ) qa_html_theme_base::nav_list($navigation, $class, $level);
	}
	...
	function doctype(){
		...
		if ( ($qa_request == 'admin/theme_options') and (qa_get_logged_in_level()>=QA_USER_LEVEL_ADMIN) ) {
			...
			// Option Form
			$options= array(
				'ok' => $saved ? 'Stylish Theme\'s settings are saved' : null,
				'tags' => 'METHOD="POST" ACTION="'.qa_path_html(qa_request()).'"',
				'style' => 'wide', // wide , tall
				'fields' => array(
					array(
						'type' => 'static',
						'label' => '<span class="qa-option-header"> Excerpt in question list </span>',
					),
					array(
					...
					array(
						'type' => 'static',
						'label' => '<span class="qa-option-header"> Attribution Links </span>',
					),
				),
